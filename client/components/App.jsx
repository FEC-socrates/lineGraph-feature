...
var localAPIpath;
var publicAPIpath = 'http://robinshood-linegraph-dev.us-west-2.elasticbeanstalk.com/';
// If localAPIpath is defined, app will localAPIpath, else it will use the publicAPIpath.
// localAPIpath = 'http://localhost:3456/'

var apiPath = localAPIpath ? localAPIpath : publicAPIpath;
...
class App extends React.Component {
  ...
  // Makes a get request to the provided path for a randomly generated stock, and then invokes any provided callback
    ...
    axios.get(`${apiPath}stocks/`)
      .then(({data}) => { 
        ...
        axios.get(`${apiPath}stocks/${ticker}`)
          .then(({data}) => {
            this.setState({
        ...
        axios.get(`${apiPath}stocks/${ticker}/${path}`)
          .then(({data}) => { 
  ...
  // Makes a get request specifically for yesterday's close price, and then invokes any provided callback
    axios.get(`${apiPath}stocks/${this.state.ticker}/yesterdayClose/`)
